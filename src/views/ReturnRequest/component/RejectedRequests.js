...
const RejectedRequests = () => {
  const rejectedRequest = useSelector((store) => store.returnRequest.returnRequest);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(getReturnRequest("rejected"));
    dispatch(setCurrentReturnOrderPage("rejected_request"))
  }, [dispatch]);
  ...
  return (
    <CCard>
      {rejectedRequest&& (<ReactTable data ={rejectedRequest?.data || []} columns = {columns}/>)}
    </CCard>
  );
};
